...
export const createWorldRenderer = (canvas: HTMLCanvasElement) => {
  ...
  let geometries: Awaited<ReturnType<typeof getModelsGeometry>>["models"] = [];
  let groundMesh: Mesh;
  let rainbowRibbonMesh: Mesh;

  const geometryPromise = getModelsGeometry().then(({ models, palette }) => {
    geometries = models;
    renderer = createRenderer(canvas, geometries, [createSphereGeometry(3)], palette);
  ...
  const getHunterScreenPos = (playerId: string) => {
    ...
    const projectedPoint = new Float32Array(3);

    mat4.multiply(viewProjMatrix, renderer.projectionMatrix, renderer.viewMatrix);

    vec3.set(projectedPoint, p.p[0], p.p[1], 0.01);
  ...
  const step = (state: ReturnType<typeof createGameSync>, playerId: string) => {
    ...
    // ground follows the player, rebuilt only when they drift far enough
    {
      ...
      if (p && state.map && Math.abs(p.p[1] - renderedGroundOrigin) > 16) {
        renderedGroundOrigin = Math.round(p.p[1]);
    ...
    // camera
    {
      ...
      vec3.set(v, player.p[0], player.p[1] - altitude * 0.6, altitude);
...

    {
      ...
      renderedWorldSnapshot.unicorns.forEach((u) => {
        ...
        quat.fromEuler(q, 0, 0, (Math.atan2(u.d[0], -u.d[1]) / Math.PI) * 180);
      ...
      renderedWorldSnapshot.hunters.forEach((h, i) => {
        const q = new Float32Array(4) as quat;
        quat.fromEuler(q, 0, 0, (Math.atan2(h.d[0], -h.d[1]) / Math.PI) * 180);
